...
class CounselingController extends Controller
{
    public function showDoctors(Request $request)
    {
        ...
        // Tambahkan filter ini untuk hanya menampilkan dokter yang sudah diverifikasi
    ...
    public function showPayment($doctor_id)
    {
        ...
        $payment = Payment::updateOrCreate(
            ['user_id' => $user->id_user,'doctor_id' => $doctorProfile->id,'status' => 'pending',],
            ['amount' => $amount,'method' => 'Midtrans',]
        );
        Consultation::firstOrCreate(
            ['id_user' => $user->id_user, 'id_dokter' => $doctorProfile->id, 'status' => 'menunggu',],
            ['tanggal_konsultasi' => now()->toDateString(), 'jam_mulai' => now()->format('H:i:s'), 'jam_selesai' => now()->addMinutes(30)->format('H:i:s'),]
        );
        ...
        $params = [
            'transaction_details' => ['order_id' => $payment->id . '-' . time(),'gross_amount' => $payment->amount,],
            'customer_details' => ['first_name' => $user->nama_user,'email' => $user->email_user,'phone' => $user->nomor_telepon ?? '081234567890',],
        ];
        try {
        ...
        return view('counseling.payment', ['payment' => $payment,'doctor' => $userDoctor,'snapToken' => $snapToken,]);
    }
    public function processPayment(Request $request, $doctor_id)
    {
        ...
        Payment::create([
            'user_id' => $user->id_user,'doctor_id' => $doctor->id_user,
            'amount' => $doctorDetail->consultation_price,'method' => $request->method,
            'status' => 'pending','created_at' => now(),'updated_at' => now(),
        ]);
        return redirect()->route('chat', $doctor_id)->with('success', 'Pembayaran berhasil disimpan.');
    }
    ...
    public function storeReview(Request $request, $consultId)
    {
        $user = Auth::user();
        Review::create([
            'id_user' => $user->id_user,'id_dokter' => $request->dokterId,
            'id_konsul' => $consultId,'tanggal_review' => now()->format('Y-m-d'),
            'waktu_review' => now()->format('H:i:s'),'rating' => $request->rating,
            'deskripsi_review' => $request->deskripsi_review,'created_at' => now(),'updated_at' => now(),
        ]);
    ...
    public function editReview(Request $request, $reviewId)
    {
        ...
        $review->update([
            'tanggal_review' => now()->format('Y-m-d'),'waktu_review' => now()->format('H:i:s'),
            'rating' => $request->rating,'deskripsi_review' => $request->deskripsi_review,
            'updated_at' => now(),
        ]);
